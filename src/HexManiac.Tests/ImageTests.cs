...
namespace HavenSoft.HexManiac.Tests {
   public class ImageTests : BaseViewModelTestClass {
      ...
      [Fact]
      public void CanAddUncompressed6BppImageRunFromViewPort() {
         // 6bpp: 64 colors, pixel value doubles as (palette page << 4 | index) -
         // pairs with a ucp4:0123-style 4-page, 16-color palette (no new palette format).
         ViewPort.Edit("^sprite`ucs6x2x2`");
         var run = Model.GetNextRun(0);
         Assert.Equal(8 * 6 * 2 * 2, run.Length); // 48 bytes/tile * 4 tiles = 192
         Assert.IsAssignableFrom<ISpriteRun>(run);
      }

      [Fact]
      public void CanCopyUncompressed6BppSprite() {
         ViewPort.Edit("^run`ucs6x1x1`");
         var fileSystem = new StubFileSystem();
         ViewPort.ExpandSelection(0, 0);
         ViewPort.Copy.Execute(fileSystem);
         var emptyBlock = " ".Join(48.Range().Select(i => "00"));
         Assert.StartsWith($"^run`ucs6x1x1` {emptyBlock}", fileSystem.CopyText);
      }

      [Fact]
      public void SixBppSpritePixels_SetThenGetRoundTrips_AllSixtyFourValues() {
         // 2x2 tiles (16x16px) gives exactly 256 pixels - enough to place all 64 distinct
         // 6-bit values 4 times each, exercising every (page, index) and every bit-group
         // position (each tile's 16 3-byte groups land on every possible bit alignment).
         ...
         var data = new byte[8 * 6 * 2 * 2]; // 2x2 tiles
         SpriteRun.SetPixels(data, 0, pixels, 6);
         var roundTripped = SpriteRun.GetPixels(data, 0, 2, 2, 6);

         Assert.Equal(pixels, roundTripped);
      }

      [Fact]
      public void SixBppSpritePixels_SpecificByteLayout_MatchesChannelerAdvanceReference() {
         // 4 pixels (0, 21, 42, 63) packed little-endian, pixel0 in the lowest 6 bits,
         // per Channeler Advance's encode_gba_tile_6bpp_from_indices - verified
         // independently (not by hand) against that reference implementation.
         var pixels = new int[8, 8];
         pixels[0, 0] = 0; pixels[1, 0] = 21; pixels[2, 0] = 42; pixels[3, 0] = 63;

         var data = new byte[48];
         SpriteRun.SetPixels(data, 0, pixels, 6);

         Assert.Equal(0x40, data[0]);
         Assert.Equal(0xA5, data[1]);
         Assert.Equal(0xFE, data[2]);
      }

      [Fact]
      public void SixBppSpritePixels_FullTile_MatchesChannelerAdvanceReferenceByteForByte() {
         // a full 8x8 tile (all 64 pixel slots, non-trivial/non-sequential values covering
         // every row and both 4-pixel groups per row), with the expected bytes generated by
         // running Channeler Advance's actual encode_gba_tile_6bpp_from_indices and pasted
         // in verbatim - not hand-computed - so this catches row/group-boundary mistakes a
         // single 4-pixel spot-check wouldn't.
         var indices = new[] { 3, 10, 17, 24, 31, 38, 45, 52, 59, 2, 9, 16, 23, 30, 37, 44, 51, 58, 1, 8, 15, 22, 29, 36, 43, 50, 57, 0, 7, 14, 21, 28, 35, 42, 49, 56, 63, 6, 13, 20, 27, 34, 41, 48, 55, 62, 5, 12, 19, 26, 33, 40, 47, 54, 61, 4, 11, 18, 25, 32, 39, 46, 53, 60 };
         var expectedBytes = new byte[] { 0x83, 0x12, 0x61, 0x9f, 0xd9, 0xd2, 0xbb, 0x90, 0x40, 0x97, 0x57, 0xb2, 0xb3, 0x1e, 0x20, 0x8f, 0xd5, 0x91, 0xab, 0x9c, 0x03, 0x87, 0x53, 0x71, 0xa3, 0x1a, 0xe3, 0xbf, 0xd1, 0x50, 0x9b, 0x98, 0xc2, 0xb7, 0x5f, 0x30, 0x93, 0x16, 0xa2, 0xaf, 0xdd, 0x13, 0x8b, 0x94, 0x81, 0xa7, 0x5b, 0xf3 };

         var pixels = new int[8, 8];
         for (int y = 0; y < 8; y++) for (int x = 0; x < 8; x++) pixels[x, y] = indices[y * 8 + x];

         var data = new byte[48];
         SpriteRun.SetPixels(data, 0, pixels, 6);
         Assert.Equal(expectedBytes, data);

         var roundTripped = SpriteRun.GetPixels(expectedBytes, 0, 1, 1, 6);
         for (int y = 0; y < 8; y++) for (int x = 0; x < 8; x++) Assert.Equal(indices[y * 8 + x], roundTripped[x, y]);
      }
